...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log('Starting Nations sync from Renderz API...');

    // 1. Fetch Nations data from Renderz API
    const renderzResponse = await fetch(
      'https://renderz.app/api/filter/filter-data/nations?seasonId=24',
    ...
    const nationsData: NationData[] = renderzData.data || renderzData;
    console.log(`Fetched ${nationsData.length} nations from Renderz`);

    // 2. Extract unique localization keys
    const localizationKeys = [...new Set(
      nationsData.map(nation => nation.name).filter(Boolean)
    )];
    ...
    // 5. UPSERT into nations table
    const { data: upsertedData, error: upsertError } = await supabase
      .from('nations')
      .upsert(transformedNations, { onConflict: 'id' });
    ...
    console.log('Nations sync completed successfully');

    return new Response(
      JSON.stringify({
        success: true,
        message: `Successfully synced ${transformedNations.length} nations`,
        synced: transformedNations.length,
    ...
    console.error('Error in sync-renderz-nations:', error);
    return new Response(
      JSON.stringify({
        success: false,

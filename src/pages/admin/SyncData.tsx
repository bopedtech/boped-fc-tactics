...
export default function SyncData() {
  ...
  const [syncingPlayerDetails, setSyncingPlayerDetails] = useState(false);
  const [playerDetailsResult, setPlayerDetailsResult] = useState<any>(null);
  ...
  const handleSyncPlayerDetails = async (mode: 'batch' | 'all' = 'batch') => {
    try {
      setSyncingPlayerDetails(true);
      setPlayerDetailsResult(null);
      
      let totalSynced = 0;
      let totalErrors = 0;
      let hasMore = true;
      let iterations = 0;
      const MAX_ITERATIONS = mode === 'all' ? 1000 : 1; // 1000 iterations max for "all" mode
      
      toast.info(`Đang đồng bộ chi tiết cầu thủ (${mode === 'all' ? 'toàn bộ' : '15 cầu thủ'})...`);

      while (hasMore && iterations < MAX_ITERATIONS) {
        iterations++;
        
        const { data, error } = await supabase.functions.invoke('sync-player-details');
        ...
        // Check if there are more players to sync
        hasMore = data.synced > 0;
        
        // Update result display
        setPlayerDetailsResult({
          success: true,
          synced: totalSynced,
          errors: totalErrors,
          message: hasMore ? `Đang tiếp tục... (${totalSynced} cầu thủ)` : 'Hoàn tất!'
        });
        
        if (mode === 'batch' || !hasMore) break;
        
        // Show progress for "all" mode
        if (hasMore && mode === 'all') {
          toast.info(`Đã đồng bộ ${totalSynced} cầu thủ, tiếp tục...`);
      ...
      if (totalSynced > 0) {
        ...
        toast.info('Tất cả cầu thủ đã có dữ liệu chi tiết!');
      }
      ...
      setPlayerDetailsResult({
        ...
        synced: totalSynced,
        errors: totalErrors,
        message: `Đã đồng bộ ${totalSynced} cầu thủ${totalErrors > 0 ? `, ${totalErrors} lỗi` : ''}`
      });
    } catch (error) {
      console.error("Error syncing player details:", error);
      toast.error("Lỗi khi đồng bộ chi tiết cầu thủ: " + (error as Error).message);
    } finally {
      setSyncingPlayerDetails(false);
    }
  };
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        {/* Sync Player Details - NEW */}
        <Card className="border-primary/50">
          <CardHeader>
            <div className="flex items-center gap-2">
              <Database className="w-5 h-5 text-primary" />
              <CardTitle>Đồng Bộ Chi Tiết Cầu Thủ</CardTitle>
            </div>
            <CardDescription>
              Đồng bộ chỉ số chi tiết, giá cả, evolution từ Renderz (15 cầu thủ/lần)
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="flex gap-2">
              <Button 
                onClick={() => handleSyncPlayerDetails('batch')} 
                disabled={syncingPlayerDetails} 
                variant="outline"
                className="flex-1"
              >
                {syncingPlayerDetails ? (
                  <Loader2 className="w-4 h-4 mr-2 animate-spin" />
                ) : (
                  <Play className="w-4 h-4 mr-2" />
                )}
              ...
              <Button 
                onClick={() => handleSyncPlayerDetails('all')} 
                disabled={syncingPlayerDetails}
                className="flex-1"
              >
                {syncingPlayerDetails ? (
            ...
            {playerDetailsResult && (
              <div className={`p-4 rounded-lg ${playerDetailsResult.success ? 'bg-green-50 dark:bg-green-950' : 'bg-red-50 dark:bg-red-950'}`}>
                <p className="text-sm font-semibold">
                  {playerDetailsResult.success ? '✅ Thành công' : '❌ Lỗi'}
                </p>
                {playerDetailsResult.synced !== undefined && (
                  <div className="text-xs space-y-1 mt-2">
                    <p>• Đã đồng bộ: {playerDetailsResult.synced} cầu thủ</p>
                    {playerDetailsResult.errors > 0 && (
                      <p className="text-red-500">• Lỗi: {playerDetailsResult.errors}</p>
                    )}
                  </div>
                )}
                {playerDetailsResult.message && (
                  <p className="text-xs mt-1 text-muted-foreground">{playerDetailsResult.message}</p>
                )}
              </div>
            )}
          </CardContent>
